...
original_df <- original_df %>%
  mutate(dxgroupFK = as.numeric(as_factor(dxgroupFK)),  
         gender = as.numeric(as_factor(gender)))
...
```{r}
new_df <- merge(df, original_df, by = c("stdyptid", 'agevisit', 'age_prediction', 'ageprediction_BrainAgeR')) 
new_df<- new_df %>%
  rename(visit = visit.y, gender = gender.y, 
         dxgroupFK = dxgroupFK.y)%>%
  dplyr::select(-visit.x, -dxgroupFK.x, -gender.x) %>%
  ...
  gender = case_when(
    gender == 1 ~ 0,
    gender == 2 ~ 1, 
    TRUE ~ gender  # keep original value if not 1 or 2
  ))  
```
...
v_sa_tSelect <- function(df, csv_filename_of_corr_boad_and_regions, hemi, bpad_type = "ridge") {
  
  bpad_col <- paste0("bpad_", bpad_type)
  
  # Select columns in alphabetical order, relocate bpad, dxgroupFK to the last columns,
  # drop the stdyptid column, and remove rows with missing values
  FS_L_corrBPAD <- df %>%
    dplyr::select(order(colnames(.))) %>%
    relocate(stdyptid, all_of(bpad_col), dxgroupFK, .after = last_col()) %>%
    dplyr::select(-stdyptid) %>%
    drop_na() %>%
    
    # Group the data by dxgroupFK and calculate the correlation of each column with the selected bpad
    group_by(dxgroupFK) %>%
    summarise_all(~cor(., .data[[bpad_col]])) %>%
    
    # Rename column names to include "_corr_bpad_{type}" except for bpad and dxgroupFK
    rename_at(vars(-all_of(c(bpad_col, "dxgroupFK"))), ~paste0(., "_corr_bpad_", bpad_type)) %>%
    
    # Rename columns to include DKregions and bpad
    set_names(c('dxgroupFK', DKregions , bpad_col)) %>%
    melt() %>%
    
    # Convert to proper types and rename variable column to region
    mutate(variable = as.character(variable), 
           value = as.numeric(value)) %>%
    rename(region = variable) %>%
    filter(region != bpad_col)
  
  # Hemisphere-specific cortical positions
  if (tolower(hemi) == "l") {
...
#original_df_unique <- original_df %>%  dplyr::select( -ends_with("age"), -ends_with('BrainAge'))# dplyr::distinct(stdyptid, .keep_all = TRUE)  # Keep rows with unique studyptid
#original_df_unique <-
```

# ASEG BRAIN PAD DF
```{r}
aseg_brain_df <- new_df %>% dplyr::select( dxgroupFK, stdyptid, starts_with("age"), starts_with('bpad') , ends_with('BrainAge'), starts_with(c('Left_', 'Right_', 'Third_', 'Fourth_Ventricle', 'Brain_Stem'))) 

aseg_df <- aseg_brain_df %>% mutate(dxgroupFK = as.factor(dxgroupFK))  %>% 
 ...
 )  %>% relocate(   bpad_ridge, .after = last_col()) %>%   drop_na %>% rename(`3rd ventricle` = Third_Ventricle, `4th ventricle` = Fourth_Ventricle, `brain stem` = Brain_Stem) 
 
```

# ASEG BRAIN PAD PHOTON AI DF
```{r}

# photon
aseg_photon_df <- aseg_brain_df %>% mutate(dxgroupFK = as.factor(dxgroupFK))  %>% 
 dplyr::select(dxgroupFK, bpad_photon , 
   Left_Amygdala, Right_Amygdala,
   Left_Cerebellum_White_Matter, Right_Cerebellum_White_Matter,
   Left_Cerebellum_Cortex, Right_Cerebellum_Cortex,
   Left_Thalamus_Proper, Right_Thalamus_Proper,
   ...
   Left_Cerebellum_White_Matter, Right_Cerebellum_White_Matter,
   Left_Cerebellum_Cortex, Right_Cerebellum_Cortex, Third_Ventricle, Fourth_Ventricle, Brain_Stem #%>%    dplyr::select( dxgroupFK, bpad_PLS_BrainAge , starts_with("Left_")) 
 )  %>% relocate(   bpad_photon, .after = last_col()) %>%   drop_na %>% rename(`3rd ventricle` = Third_Ventricle, `4th ventricle` = Fourth_Ventricle, `brain stem` = Brain_Stem) 
...
# Subcortical region correlation with brain-pad Ridge Multimodal
...
#ggsave('output/figures/aseg_subcortical_structures_volume_corr_bpad_photon.jpeg')

```


# Subcortical region correlation with brain-pad PHOTON AI
```{r}

melt_aseg_photon_df <- aseg_photon_df %>%
  # Group the data by dxgroupFK and calculate the correlation of each column with bpad_PLS_BrainAge  #%>%  rename_at(vars(  -dxgroupFK), ~paste0( ., "_corr_bpad_pls")) 
  group_by(dxgroupFK) %>% summarise_all(~cor(., bpad_photon)) %>% dplyr::select(-bpad_photon)  %>% tidyr::pivot_longer(-dxgroupFK, names_to = "variable" ) %>%
  unnest(value) %>% 
  mutate(hemi = ifelse(str_detect(variable, "^Left_"), "left", "right"),  variable = str_replace(variable, "^Left_|^Right_", "") ) %>%   mutate(variable = ifelse(variable != "VentralDC", tolower(str_replace_all(variable, "_", " ")), "ventral DC"),hemi = ifelse(variable %in% c('4th ventricle', '3rd ventricle', 'brain stem'),   "midline", hemi)) %>% 
     # rename the variable column to region,
    rename(region = variable) 
merged_photon_df <- merge(melt_aseg_photon_df, aseg$data)
merged_photon_df %>% write.csv(., 'output/data/aseg_subcortical_structures_volume_corr_bpad_photon.csv')


merged_photon_df %>%group_by(dxgroupFK) %>% ggplot(aes(fill = value)) + geom_brain(atlas = aseg) +  scale_fill_continuous_divergingx(palette = 'RdBu', mid = 0 , limits = c(-0.75, 0.75))+theme_void()+facet_wrap(~dxgroupFK) 
##ggsave('output/figures/aseg_subcortical_structures_volume_corr_bpad_photon.jpeg')
...
FSvolume_L <- merge(original_df_unique_volume_left, only_bpad_ridge_df, by = c('stdyptid')) %>% filter(rowSums(is.na(.)) <= 7) %>% mutate(dxgroupFK = as.factor(dxgroupFK)) 

only_bpad_ridge_df <- df %>% dplyr::select(bpad_ridge, stdyptid)
FSvolume_R <- merge(original_df_unique_volume_right, only_bpad_ridge_df, by = c('stdyptid')) %>% filter(rowSums(is.na(.)) <= 7) %>% mutate(dxgroupFK = as.factor(dxgroupFK)) 
...
v_sa_tSelect(FSvolume_L,"output/data/freesurfer_volume_corr_bpad_left.csv", "l", 'ridge')
...
### Create brain map of correlation between volume and Brain-PAD PHOTON AI for left and right hemi
```{r}
original_df_unique_volume_left <- original_df %>%  dplyr::select(  ends_with("L_V"), -ends_with("age"),stdyptid, dxgroupFK, -Banks_L_V, -HC_combined_L_V)
original_df_unique_volume_right <- original_df %>%  dplyr::select(  ends_with("R_V"), -ends_with("age"),stdyptid, dxgroupFK, -HC_combined_R_V) # there is NO Banks_R_V
only_bpad_photon_df <- df %>% dplyr::select(bpad_photon, stdyptid)


FSvolume_L_photon <- merge(original_df_unique_volume_left, only_bpad_photon_df, by = c('stdyptid')) %>% filter(rowSums(is.na(.)) <= 7) %>% mutate(dxgroupFK = as.factor(dxgroupFK)) 
FSvolume_R_photon <- merge(original_df_unique_volume_right, only_bpad_photon_df, by = c('stdyptid')) %>% filter(rowSums(is.na(.)) <= 7) %>% mutate(dxgroupFK = as.factor(dxgroupFK)) 

FSvolume_L_photon %>% write.csv("output/data/freesurfer_gray_volume_left_photon.csv")
FSvolume_R_photon %>% write.csv("output/data/freesurfer_gray_volume_right_photon.csv")

#v_sa_tSelect(FSvolume_L_photon,"output/data/fre esurfer_volume_corr_bpad_photon_left.csv", "l", bpad_type = "photon")
#ggsave("output/figures/FSvolume_L_corr_brainpad_photon.png")

#v_sa_tSelect(FSvolume_R_photon,"output/data/freesurfer_volume_corr_bpad_photon_right.csv", "r",  bpad_type = "photon")
#ggsave("output/figures/FSvolume_R_corr_brainpad_photon.png")
```
...
original_df_unique_surface_area_left <- original_df %>%  dplyr::select(ends_with("L_S"), -ends_with("age"), stdyptid, dxgroupFK,-Banks_STS_L_S)
original_df_unique_surface_area_right <- original_df %>%  dplyr::select(ends_with("R_S"), -ends_with("age"), stdyptid, dxgroupFK)

original_df_unique_surface_area_right

FSsurfarea_L <-merge(original_df_unique_surface_area_left, only_bpad_ridge_df, by = c('stdyptid')) %>% filter(rowSums(is.na(.)) <= 7) %>% mutate(dxgroupFK = as.factor(dxgroupFK)) 
FSsurfarea_R <-merge(original_df_unique_surface_area_right, only_bpad_ridge_df, by = c('stdyptid')) %>% filter(rowSums(is.na(.)) <= 7) %>% mutate(dxgroupFK = as.factor(dxgroupFK)) 


FSsurfarea_L

v_sa_tSelect(FSsurfarea_L,"output/data/freesurfer_surface_area_corr_bpad_left.csv", "l",'ridge')
ggsave("output/figures/FSsurface_area_L_corr_brainpad_Ridge.png")

v_sa_tSelect(FSsurfarea_R,"output/data/freesurfer_surface_area_corr_bpad_right.csv", "r",'ridge')
...
FSsurfarea_photon_L <-merge(original_df_unique_surface_area_left, only_bpad_photon_df, by = c('stdyptid')) %>% filter(rowSums(is.na(.)) <= 7) %>% mutate(dxgroupFK = as.factor(dxgroupFK)) 
FSsurfarea_photon_R <-merge(original_df_unique_surface_area_right, only_bpad_photon_df, by = c('stdyptid')) %>% filter(rowSums(is.na(.)) <= 7) %>% mutate(dxgroupFK = as.factor(dxgroupFK)) 
 
v_sa_tSelect(FSsurfarea_photon_L,"output/data/freesurfer_surface_area_corr_bpad_photon_left.csv", "l",'photon')
v_sa_tSelect(FSsurfarea_photon_R,"output/data/freesurfer_surface_area_corr_bpad_photon_right.csv", "l",'photon')
...
FSthickness_ridge_L <-merge(FSthickness_L, only_bpad_ridge_df, by = c('stdyptid')) %>% filter(rowSums(is.na(.)) <= 7) %>% mutate(dxgroupFK = as.factor(dxgroupFK)) 
...
FSthickness_ridge_R <-merge(FSthickness_R, only_bpad_ridge_df, by = c('stdyptid')) %>% filter(rowSums(is.na(.)) <= 7) %>% mutate(dxgroupFK = as.factor(dxgroupFK)) 

v_sa_tSelect(FSthickness_ridge_L, "output/data/freesurfer_thickness_corr_bpad_ridge_left.csv", "l", 'ridge')
ggsave("output/figures/FSsthickness_L_corr_brainpad_Ridge.png")


v_sa_tSelect(FSthickness_ridge_R , "output/data/freesurfer_thickness_corr_bpad_ridge_right.csv", "r", 'ridge')
...
FSthickness_L <- original_df %>% dplyr::select(stdyptid, dxgroupFK , caudalanteriorcingulate_L_T, ends_with("_L")) %>% dplyr::select(-c('totalavgCT_L', 'totalSA_L', 'totalGMvolume_L', 'Banks_L' ))
FSthickness_photon_L <-merge(FSthickness_L, only_bpad_photon_df, by = c('stdyptid')) %>% filter(rowSums(is.na(.)) <= 7) %>% mutate(dxgroupFK = as.factor(dxgroupFK)) 

FSthickness_R <- original_df %>% dplyr::select(stdyptid, dxgroupFK ,  ends_with("_R")) %>% dplyr::select(-c('totalavgCT_R', 'totalSA_R', 'totalGMvolume_R', 'Banks_R','basis_r'))
FSthickness_photon_R <-merge(FSthickness_R, only_bpad_photon_df, by = c('stdyptid')) %>% filter(rowSums(is.na(.)) <= 7) %>% mutate(dxgroupFK = as.factor(dxgroupFK)) 


v_sa_tSelect(FSthickness_photon_L, "output/data/freesurfer_thickness_corr_bpad_photon_left.csv", "l", 'photon')
ggsave("output/figures/FSsthickness_L_corr_brainpad_photon.png")


v_sa_tSelect(FSthickness_photon_R , "output/data/freesurfer_thickness_corr_bpad_photon_right.csv", "r", 'photon')
ggsave("output/figures/FSsthickness_R_corr_brainpad_photon.png")
FSthickness_photon_L
